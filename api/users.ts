...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    switch (action) {
      ...
      case 'batch-get':
        return await handleBatchGet(req, res);
      default:
...
// GET /api/users?action=batch-get&userIds=id1,id2
async function handleBatchGet(req: VercelRequest, res: VercelResponse) {
  const userIdsStr = (req.query.userIds as string) || '';
  const userIds = userIdsStr.split(',').filter(Boolean);

  if (userIds.length === 0) {
    return success(res, { userInfoMap: {} });
  }
  ...
  try {
    ...
    return success(res, { userInfoMap: results });
  } catch (err: any) {
    console.error('批量获取用户失败:', err);
    return error(res, err.message || '获取用户信息失败');
  }
}
