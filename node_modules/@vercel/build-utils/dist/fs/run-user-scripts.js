...
function isPnpmMajorAvailable(major) {
  if (major <= 10) {
    return true;
  }
  return import_fs_extra.default.existsSync(`/pnpm${major}`);
}
function nodeSupportsPnpm11Default(nodeVersion) {
  if (!nodeVersion || (0, import_node_version.isBunVersion)(nodeVersion)) {
    return true;
  }
  return nodeVersion.major >= 22;
}
function detectPnpmVersion(lockfileVersion, projectCreatedAt, nodeVersion) {
  switch (true) {
    ...
    case lockfileVersion === 9: {
      if (projectCreatedAt && projectCreatedAt >= PNPM_11_PREFERRED_AT.getTime() && nodeSupportsPnpm11Default(nodeVersion) && isPnpmMajorAvailable(11)) {
        return "pnpm 11";
      }
      if (projectCreatedAt && projectCreatedAt >= PNPM_10_PREFERRED_AT.getTime()) {
        return "pnpm 10";
      }
      return "pnpm 9";
    }
...
function lockfileCompatibleWithPnpm(lockfileVersion, packageManagerVersion) {
...
function highestInstalledPnpmMajor(versionRange, lockfileVersion) {
  if (!(0, import_semver.validRange)(versionRange)) {
    return void 0;
  }
  for (const major of INSTALLED_PNPM_MAJORS) {
    ...
    if (!(0, import_semver.intersects)(`${major}.x`, versionRange)) {
      continue;
    }
    const versionForLockfile = (0, import_semver.parse)(major === 9 ? "9.0.1" : `${major}.0.0`);
    if (!versionForLockfile || !lockfileCompatibleWithPnpm(lockfileVersion, versionForLockfile)) {
      continue;
    }
    return major;
  }
...
function resolveDevEnginesPnpmRange(devEngines) {
  if (!devEngines?.packageManager) {
    return void 0;
  }
  const entries = Array.isArray(devEngines.packageManager) ? devEngines.packageManager : [devEngines.packageManager];
...
function resolveCompatiblePnpmPin({
  ...
  if (parsed?.packageName === "pnpm") {
    if (lockfileCompatibleWithPnpm(lockfileVersion, parsed.packageVersion) && isPnpmMajorAvailable(parsed.packageVersion.major)) {
      return {
...
function applyEnginesPnpm({
  ...
  const selectedRange = selected && "pnpmVersionRange" in selected ? selected.pnpmVersionRange : void 0;
  if (selectedRange && (0, import_semver.validRange)(selectedRange) && (0, import_semver.validRange)(enginesPnpm)) {
    if ((0, import_semver.intersects)(selectedRange, enginesPnpm)) {
      if (!usedPin) {
...
function detectPackageManager(cliType, lockfileVersion, projectCreatedAt, nodeVersion) {
  switch (cliType) {
    case "npm":
      return void 0;
    case "pnpm": {
