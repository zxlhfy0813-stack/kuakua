...
const InternalTransfer = (props, ref) => {
  ...
  const setStateKeys = (0, _react.useCallback)((direction, keys) => {
    if (direction === 'left') {
      setSourceSelectedKeys((0, _is.isFunction)(keys) ? keys(sourceSelectedKeys || []) : keys);
    } else {
      setTargetSelectedKeys((0, _is.isFunction)(keys) ? keys(targetSelectedKeys || []) : keys);
    }
  }, [sourceSelectedKeys, targetSelectedKeys]);
  ...
  const leftActive = rightDataSource.some(data => (0, _is.isNonNullable)(data.key) && targetSelectedKeys.includes(data.key) && !data.disabled);
  const rightActive = leftDataSource.some(data => (0, _is.isNonNullable)(data.key) && sourceSelectedKeys.includes(data.key) && !data.disabled);
  // ====================== Styles ======================
  ...
  const getMergedSectionClassNames = direction => {
    ...
    sectionSemanticKeys.forEach(key => {
      mergedSectionClassNames[key] = (0, _clsx.clsx)(mergedClassNames[key], directionClassNames?.[key]);
    });
...
const Transfer = /*#__PURE__*/_react.default.forwardRef(InternalTransfer);
if (process.env.NODE_ENV !== 'production') {
