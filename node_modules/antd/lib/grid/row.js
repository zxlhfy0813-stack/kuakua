...
const useMergedPropByScreen = (oriProp, screen) => {
  const [prop, setProp] = React.useState(() => (0, _is.isString)(oriProp) ? oriProp : '');
  const calcMergedAlignOrJustify = () => {
    if ((0, _is.isString)(oriProp)) {
      setProp(oriProp);
    }
    if (!(0, _is.isPlainObject)(oriProp)) {
      return;
    }
...
const Row = /*#__PURE__*/React.forwardRef((props, ref) => {
  ...
  const rowStyle = {};
  if (gutters?.[0]) {
    const horizontalGutter = (0, _is.isNumber)(gutters[0]) ? `${gutters[0] / -2}px` : `calc(${gutters[0]} / -2)`;
