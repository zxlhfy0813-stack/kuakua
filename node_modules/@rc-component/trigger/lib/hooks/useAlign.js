...
function useAlign(open, popupEle, target, placement, builtinPlacements, popupAlign, onPopupAlign, mobile) {
  ...
  const scrollerList = React.useMemo(() => {
    ...
    return (0, _util2.collectScroller)(popupEle);
  }, [popupEle]);
  ...
  // ========================= Align =========================
  const onAlign = (0, _util.useEvent)(() => {
    if (popupEle && target && open && !mobile) {
      ...
      const scrollRegionArea = (0, _util2.getVisibleArea)(scrollRegion, scrollerList);
      const visibleRegionArea = (0, _util2.getVisibleArea)(visibleRegion, scrollerList);
      ...
      // Calculate scale
      const scaleX = (0, _util2.toNum)(Math.round(popupWidth / parseFloat(width) * 1000) / 1000);
      const scaleY = (0, _util2.toNum)(Math.round(popupHeight / parseFloat(height) * 1000) / 1000);

      // No need to align since it's not visible in view
      if (scaleX === 0 || scaleY === 0 || (0, _util.isDOM)(target) && !(0, _util.isVisible)(target)) {
        return;
      }
  ...
  (0, _util.useLayoutEffect)(resetReady, [placement]);
